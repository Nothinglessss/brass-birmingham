...
const source = ['gameData.js', 'uiManager.js']
...
function testSuccessfulSellContinuesWithRefreshedTargets() {
    ...
    assert.equal(typeof ui.handleSellResult, 'function');
    ...
    assert.deepEqual(Array.from(ui.sellSession.messages), ['Sold Manufacturer Lv1']);
...
function testDoneSellingFooterOnlyAppearsAfterCommittedSale() {
    const ui = new context.UIManager();
    assert.equal(typeof ui.renderSellFooterHtml, 'function');
